  ...
  const fetchInactiveStudents = useCallback(async () => {
    ...
    try {
      ...
      const [inactiveRes, blockedRes, archivedRes] = await Promise.allSettled([
        studentsApi.getAll({ status: 'inactive', ...(instituteId && { institute_id: instituteId }) }),
        studentsApi.getAll({ status: 'blocked',  ...(instituteId && { institute_id: instituteId }) }),
        studentsApi.getAll({ status: 'archived', ...(instituteId && { institute_id: instituteId }) }),
      ])
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid gap-3 sm:gap-4 grid-cols-2 xl:grid-cols-4">
        {[
          ...
          { label: 'Inactive', value: rows.filter(r => r.student.status === 'inactive').length,   gradient: 'from-gray-400 to-gray-600',    border: 'hover:border-gray-500/50',    icon: UserMinus   },
